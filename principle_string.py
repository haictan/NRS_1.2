...
principle_tips = '''
*（tips: 干涉是**两列或两列以上的波**在空间中相遇时，因**叠加或抵消**而形成新的波形的现象。叠加区域内的光强不均匀分布，其**明暗程度随位置的不同而变化**。
在牛顿环干涉实验中，同一干涉环上各处空气层厚度相同，因此它属于**等厚干涉**。
干涉现象的强度取决于光的波长、介质的折射率以及两个界面的厚度差。根据等厚条件，当两个界面之间的厚度差等于光的波长的整数倍时，我们就会观察到明纹暗纹。）*
'''